...
> **★先读总交接清单**：`docs/克隆服务器交接清单.md`（2026-09-16 起）。它是最新总入口，含：
> - 5 步核心工作 + **三处地址登记（switch 脚本 / VPS 看门狗 / ssh config）**
> - Hermes 自动切换模式（`hermes_qwen_switch.py`）vs 静态指定两种
> - 当前机况速查表 + 验证清单
> 本 SKILL.md 的逐步命令照常可用，但以交接清单为准对齐最新模式（尤其 Hermes 已从「静态指新机」升级为「自动探测选在线机」）。
